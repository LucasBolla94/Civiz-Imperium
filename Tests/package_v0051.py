"""Build and verify V0.0.5.1 from a committed snapshot, excluding unrelated local edits."""
import os
import shutil
import subprocess
import zipfile
...
env['CIVIZ_CAPTURE'] = str(root / 'Tests/v0051_portable.png')
...
def run(args, cwd, tag, timeout=180):
    log = root / 'Tests' / f'package_v0051_{tag}.log'
...
for name in ['README.md', 'GODOT-LICENSE.txt', f'PLANO_V{version}.md', f'ASSETS_V{version}.md']:
...
(build / 'LEIA-ME.txt').write_text(
    ...
    'Saves V0.0.5 continuam compatíveis (civilization_v005.save).\n'
    f'Código da versão: {revision}\n', encoding='utf-8')
output = run([str(exe), '--script', str(snapshot / 'Tests/test_v0051_portable.gd')], build, 'portable', 60)
...
with zipfile.ZipFile(windows_zip, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
    for name in [exe.name, pck.name, 'LEIA-ME.txt', 'README.md', 'GODOT-LICENSE.txt', f'PLANO_V{version}.md', f'ASSETS_V{version}.md']:
