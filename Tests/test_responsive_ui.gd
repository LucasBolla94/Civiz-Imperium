...
func run() -> void:
	...
	for resolution in SIZES:
		...
		check(map_area.get_area() / root.get_visible_rect().get_area() >= 0.70,"At least 70 percent of viewport reserved for map: viewport %s, map %s"%[root.get_visible_rect(),map_area])
		...
		for controls in game.hud.activity_controls.values():
			var work_scroll: ScrollContainer=game.hud.workforce_view.get_parent()
			work_scroll.ensure_control_visible(controls.plus)
			await settle()
			check(work_scroll.get_global_rect().encloses(controls.plus.get_global_rect()),"Every allocation button reachable within the scrolling panel")
