extends RefCounted
var game
var stock_targets := {"fruit": 0, "wood": 0, "stone": 0}
var population_target := 7
...
func production_limit(resource: String) -> int:
	...
	if resource == "fruit": target = maxi(target, (game.workers.size() + 1) * game.DATA.FOOD_RESERVE_PER_PERSON)
	var requests := 0
	for site in game.buildings + game.jobs:
...
func allowed_amount(source, capacity: int) -> int:
	# Clear orchard stumps even when the wood goal is met, so fruit renewal continues.
...
func tick(delta: float) -> void:
	...
	for cell in orchards:
		...
		if game.settlement.food_units() < game.workers.size() * game.DATA.FOOD_RESERVE_PER_PERSON + game.DATA.PLANT_COST.fruit: continue
